...
global video_addrs, imgs_addrs, title, video_num, Anum
...
def choose_type_funny(html): 
	
	a = html.find('"categoryPath":"')

	return html[a+16:a+21]
...
def main():
	...
	Label(root, text="要下载搞笑视频的数量").grid(row=0, column=0)

	e1 = Entry(root)

	Label(root, text="id码").grid(row=1, column=0)

	e2 = Entry(root)

	Label(root, text="搜索范围（默认100）").grid(row=2, column=0)
	...
	def download_all():
		...
		video_num = e1.get()
		num_all = int(video_num)

		id_num = int(e2.get())

		search_num = int(e3.get())
		Anum = search_num
		...
		for i in range(Anum):
			tt = str(id_num - i)
			url_api = 'http://v.ifeng.com/video_' + tt + '.shtml'
			html = url_open(url_api).decode('UTF-8')

			videotype_funny = choose_type_funny(html)
			
			if videotype_funny == 'funny' and (videonum_k <= num_all):	
				...
				print(videonum_k,'/',num_all,'  ',i,'/',Anum,)
				videonum_k += 1
			
			if videonum_k > num_all:
				print("Done")
				break
